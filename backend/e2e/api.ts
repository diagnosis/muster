...
import {CreateOutingInput, UpdateInput, UpdateMeInput} from "./types";
import {RegisterRequest} from "./types";
import {tag} from "./fixtures";

const HOURS = [48, 72, 96, 120]
const outings: Omit<CreateOutingInput, 'starts_at'>[] = [
    { title: 'Poo Poo Point', destination: 'Issaquah', meet_label: 'South Bellevue P&R', max_size: 10, host_seats: 3, cost_per_seat_cents: 350, difficulty: 'moderate', pace: 'relaxed', notes: 'Paraglider launch views at the top' },
    { title: 'Rattlesnake Ledge', destination: 'North Bend', meet_label: 'Eastgate P&R', max_size: 8, host_seats: 4, cost_per_seat_cents: 2550, difficulty: 'easy', pace: 'relaxed', notes: 'Busy trail — early start' },
    { title: 'Mount Si', destination: 'North Bend', meet_label: 'Issaquah Highlands P&R', max_size: 6, host_seats: 3, cost_per_seat_cents: 450, difficulty: 'hard', pace: 'moderate', notes: 'Bring plenty of water, 3100 ft gain' },
    ...
    { title: 'Mailbox Peak', destination: 'North Bend', meet_label: 'South Bellevue P&R', max_size: 4, host_seats: 4, cost_per_seat_cents: 550, difficulty: 'hard', pace: 'fast', notes: 'New trail up, old trail down for the brave' },
    ...
    { title: 'Melakwa Lake', destination: 'Denny Creek', meet_label: 'Issaquah Highlands P&R', max_size: 6, host_seats: 4, cost_per_seat_cents: 450, difficulty: 'hard', pace: 'moderate', notes: 'Cross the famous waterslide slabs' },
    ...
    { title: 'Colchuck Lake', destination: 'Leavenworth', meet_label: 'Monroe P&R', max_size: 6, host_seats: 3, cost_per_seat_cents: 1050, difficulty: 'hard', pace: 'fast', notes: 'Enchantments gateway — permits for overnight only' },
...
export const createOuting = (ctx: APIRequestContext, overrides = {}) =>
    ctx.post('/api/outings', { data: { ...uniqueOuting(), ...overrides } });
